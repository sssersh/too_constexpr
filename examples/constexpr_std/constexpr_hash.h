...
#ifndef _LIBCPP___FUNCTIONAL_CONSTEXPR_HASH_H
#define _LIBCPP___FUNCTIONAL_CONSTEXPR_HASH_H
...
namespace constexpr_std { inline namespace _LIBCPP_ABI_NAMESPACE {

template<typename T1, typename T2>
using pair = std::pair<T1, T2>;

template<typename T1, typename T2>
using __unary_function = std::__unary_function<T1, T2>;

template<typename T1>
using is_enum = std::is_enum<T1>;

template<typename T1>
using underlying_type = std::underlying_type<T1>;

template<typename T1>
using is_copy_constructible = std::is_copy_constructible<T1>;

template<typename T1>
using is_move_constructible = std::is_move_constructible<T1>;

template<typename T1>
using is_default_constructible = std::is_default_constructible<T1>;

template<typename... Ts>
using __invokable_r = std::__invokable_r<Ts...>;

template<typename T1, T1 Val>
using integral_constant = std::integral_constant<T1, Val>;

template<typename T1, typename T2>
using __check_hash_requirements = std::__check_hash_requirements<T1, T2>;

template<bool... _Pred>
using __all = std::__all<_Pred...>;

template <bool Val, class _Tp = void>
using enable_if = std::enable_if<Val, _Tp>;
...
{
    constexpr inline _Size operator()(const void* __key, _Size __len)
...
{
    constexpr inline _Size operator()(const void* __key, _Size __len)  _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK;
 ...
 private:
  ...
  static constexpr _Size __rotate(_Size __val, int __shift) {
    return __shift == 0 ? __val : ((__val >> __shift) | (__val << (64 - __shift)));
  }

  static constexpr _Size __rotate_by_at_least_1(_Size __val, int __shift) {
    return (__val >> __shift) | (__val << (64 - __shift));
  }

  static constexpr _Size __shift_mix(_Size __val) {
    return __val ^ (__val >> 47);
  }

  static constexpr _Size __hash_len_16(_Size __u, _Size __v)
     _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
  {
  ...
  static constexpr _Size __hash_len_0_to_16(const char* __s, _Size __len)
     _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
  {
    if (__len > 8) {
  ...
  static constexpr _Size __hash_len_17_to_32(const char *__s, _Size __len)
     _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
  {
  ...
  static constexpr pair<_Size, _Size> __weak_hash_len_32_with_seeds(
  ...
  static constexpr _Size __hash_len_33_to_64(const char *__s, size_t __len)
    _LIBCPP_DISABLE_UBSAN_UNSIGNED_INTEGER_CHECK
  {
...
}} // namespace constexpr_std

#endif // _LIBCPP___FUNCTIONAL_CONSTEXPR_HASH_H
