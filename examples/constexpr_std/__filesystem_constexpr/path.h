...
struct __is_pathable_string<
  ...
  _LIBCPP_HIDE_FROM_ABI
  constexpr static _ECharT const* __range_begin(_Str const& __s) { return __s.data(); }
...
struct __is_pathable_char_array<_Source, _ECharT*, _UPtr, true>
  ...
  _LIBCPP_HIDE_FROM_ABI
  constexpr static _ECharT const* __range_begin(const _ECharT* __b) { return __b; }

  _LIBCPP_HIDE_FROM_ABI
  constexpr static _ECharT const* __range_end(const _ECharT* __b) {
...
struct _PathCVT {
  ...
  template <class _Iter, class value_type, template<typename> typename Allocator>
  _LIBCPP_HIDE_FROM_ABI
  constexpr static void __append_range(__path_string& __dest, _Iter __b, _Iter __e, Allocator<value_type> alloc) {
    static_assert(!is_same<_Iter, _ECharT*>::value, "Call const overload");
    if (__b == __e)
      return;
    basic_string<_ECharT, std::char_traits<value_type>, Allocator<value_type>> __tmp(__b, __e, alloc);
#if defined(_LIBCPP_WIN32API)
    string __utf8;
    _Narrower()(back_inserter(__utf8), __tmp.data(),
                __tmp.data() + __tmp.length());
  ...
  template <class _Iter>
  _LIBCPP_HIDE_FROM_ABI
  constexpr static void __append_range(__path_string& __dest, _Iter __b, _NullSentinel) {
    static_assert(!is_same<_Iter, _ECharT*>::value, "Call const overload");
    const _ECharT __sentinel = _ECharT{};
    if (*__b == __sentinel)
      return;
  ...
  _LIBCPP_HIDE_FROM_ABI
  constexpr static void __append_source(__path_string& __dest, _Source const& __s) {
...
class _LIBCPP_TYPE_VIS basic_path {
  template <class _SourceOrIter, class _Tp = basic_path&>
...
public:
...
#endif
  typedef basic_string<value_type, std::char_traits<value_type>, Allocator<value_type>> string_type;
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr basic_path() noexcept {}
  _LIBCPP_HIDE_FROM_ABI constexpr basic_path(const basic_path& __p) : __pn_(__p.__pn_) {}
  _LIBCPP_HIDE_FROM_ABI constexpr basic_path(basic_path&& __p) noexcept
      : __pn_(_VSTD::move(__p.__pn_)) {}

  _LIBCPP_HIDE_FROM_ABI
  constexpr basic_path(string_type&& __s, format = format::auto_format) noexcept
      : __pn_(_VSTD::move(__s)) {}

  template <class _Source, class = _EnableIfPathable<_Source, void> >
  _LIBCPP_HIDE_FROM_ABI
  constexpr basic_path(const _Source& __src, format = format::auto_format) {
    _SourceCVT<_Source>::__append_source(__pn_, __src);
  }

  template <class _InputIt>
  _LIBCPP_HIDE_FROM_ABI
  constexpr basic_path(_InputIt __first, _InputIt __last, Allocator<value_type> alloc = Allocator<value_type>{}, format = format::auto_format) {
    typedef typename iterator_traits<_InputIt>::value_type _ItVal;
    _PathCVT<_ItVal>::template __append_range<_InputIt, value_type, Allocator>(__pn_, __first, __last, alloc);
  }
...
*/
  ...
  // assignments
  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator=(const basic_path& __p) {
    __pn_ = __p.__pn_;
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator=(basic_path&& __p) noexcept {
    __pn_ = _VSTD::move(__p.__pn_);
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator=(string_type&& __s) noexcept {
    __pn_ = _VSTD::move(__s);
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& assign(string_type&& __s) noexcept {
    __pn_ = _VSTD::move(__s);
    return *this;
  }
  ...
  template <class _InputIt>
  _LIBCPP_HIDE_FROM_ABI
  basic_path& assign(_InputIt __first, _InputIt __last) {
    typedef typename iterator_traits<_InputIt>::value_type _ItVal;
    __pn_.clear();
...
public:
  ...
  template <class _InputIt>
  _LIBCPP_HIDE_FROM_ABI
  basic_path& append(_InputIt __first, _InputIt __last) {
    typedef typename iterator_traits<_InputIt>::value_type _ItVal;
  ...
  // concatenation
  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator+=(const basic_path& __x) {
    __pn_ += __x.__pn_;
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator+=(const string_type& __x) {
    __pn_ += __x;
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator+=(__string_view __x) {
    __pn_ += __x;
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator+=(const value_type* __x) {
    __pn_ += __x;
    return *this;
  }

  _LIBCPP_HIDE_FROM_ABI
  basic_path& operator+=(value_type __x) {
    __pn_ += __x;
    return *this;
  }
  ...
  template <class _ECharT>
  _LIBCPP_HIDE_FROM_ABI
  typename enable_if<__can_convert_char<_ECharT>::value, basic_path&>::type
  ...
  template <class _InputIt>
  _LIBCPP_HIDE_FROM_ABI
  basic_path& concat(_InputIt __first, _InputIt __last) {
    typedef typename iterator_traits<_InputIt>::value_type _ItVal;
    _PathCVT<_ItVal>::__append_range(__pn_, __first, __last);
    return *this;
  }
  ...
  _LIBCPP_HIDE_FROM_ABI
  basic_path& replace_filename(const basic_path& __replacement) {
    remove_filename();
    return (*this /= __replacement);
  }

  basic_path& replace_extension(const basic_path& __replacement = basic_path());

  friend _LIBCPP_HIDE_FROM_ABI bool operator==(const basic_path& __lhs, const basic_path& __rhs) noexcept {
...
#  else // _LIBCPP_STD_VER <= 17
  friend _LIBCPP_HIDE_FROM_ABI strong_ordering operator<=>(const basic_path& __lhs, const basic_path& __rhs) noexcept {
    return __lhs.__compare(__rhs.__pn_) <=> 0;
  }
#  endif // _LIBCPP_STD_VER <= 17

  friend _LIBCPP_HIDE_FROM_ABI basic_path operator/(const basic_path& __lhs, const basic_path& __rhs) {
    basic_path __result(__lhs);
    __result /= __rhs;
    return __result;
  }

  _LIBCPP_HIDE_FROM_ABI
  void swap(basic_path& __rhs) noexcept { __pn_.swap(__rhs.__pn_); }
...
public:
  // compare
  _LIBCPP_HIDE_FROM_ABI int compare(const basic_path& __p) const noexcept {
  ...
  // decomposition
  _LIBCPP_HIDE_FROM_ABI basic_path root_name() const {
    return string_type(__root_name());
  }
  _LIBCPP_HIDE_FROM_ABI basic_path root_directory() const {
    return string_type(__root_directory());
  }
  _LIBCPP_HIDE_FROM_ABI basic_path root_path() const {
#if defined(_LIBCPP_WIN32API)
  ...
  _LIBCPP_HIDE_FROM_ABI basic_path relative_path() const {
    return string_type(__relative_path());
  }
  _LIBCPP_HIDE_FROM_ABI basic_path parent_path() const {
    return string_type(__parent_path());
  }
  _LIBCPP_HIDE_FROM_ABI basic_path filename() const {
    return string_type(__filename());
  }
  _LIBCPP_HIDE_FROM_ABI basic_path stem() const { return string_type(__stem()); }
  _LIBCPP_HIDE_FROM_ABI basic_path extension() const {
  ...
  basic_path lexically_relative(const basic_path& __base) const;

  _LIBCPP_HIDE_FROM_ABI basic_path lexically_proximate(const basic_path& __base) const {
    basic_path __result = this->lexically_relative(__base);
  ...
  _LIBCPP_HIDE_FROM_ABI friend
      ...
      operator<<(basic_ostream<_CharT, _Traits>& __os, const basic_path& __p) {
  ...
  operator>>(basic_istream<_CharT, _Traits>& __is, basic_path& __p) {
...
private:
  inline _LIBCPP_HIDE_FROM_ABI basic_path&
...
typedef basic_path<std::allocator> path;
